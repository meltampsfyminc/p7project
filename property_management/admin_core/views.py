...
from .forms import (
    ...
    OfficeFormSet, SectionFormSet, WorkerForm, WorkerFormSet, 
    WorkerOfficeAssignmentForm, DepartmentForm, SectionForm, 
    AdminBuildingForm
)
from .models import (
...
def worker_office_create(request):
    ...
    if form.is_valid():
        form.save()
        messages.success(request, "Worker assigned to office successfully.")
...
def worker_office_update(request, pk):
    ...
    if form.is_valid():
        form.save()
        messages.success(request, "Office assignment updated successfully.")
        return redirect("admin_core:worker_office_list")
...
def housing_assignment_list(request):
    assignments = (
        ...
        .select_related("worker", "housing_unit", "housing_unit__site")
...
def housing_assignment_create(request):
    ...
    if form.is_valid():
        ...
        messages.success(request, "Worker assigned to housing unit successfully.")
...
def housing_assignment_update(request, pk):
    ...
    if form.is_valid():
        form.save()
        messages.success(request, "Housing assignment updated successfully.")
        return redirect("admin_core:housing_assignment_list")
...
def mass_workers(request):
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            formset.save()
            messages.success(request, "Workers added successfully.")
...
def mass_offices(request):
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            formset.save()
            messages.success(request, "Offices added successfully.")
